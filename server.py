...
def ms_fetch(url, extra_headers=None):
...
def search(query):
    ...
    # Endpoint 1: displaycatalog (most reliable from servers)
    try:
        url = ("https://displaycatalog.mp.microsoft.com/v7.0/products"
               f"?market=US&languages=en-US,neutral&MS-CV=0"
               f"&query={urllib.parse.quote(query)}"
               f"&deviceFamily=Windows.Xbox&itemType=Game")
        data     = ms_fetch(url)
        ...
        errors.append(f"displaycatalog: {e}")

    # Endpoint 2: storeedgefd
    try:
        ...
        errors.append(f"storeedgefd: {e}")

    raise Exception("Search failed: " + " | ".join(errors))

def _parse_games(products):
    games = []
    for p in products[:12]:
        lp    = (p.get("LocalizedProperties") or [{}])[0]
        title = lp.get("ProductTitle","")
